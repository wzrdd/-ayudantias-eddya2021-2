...
class Result {
    ...
    public static SinglyLinkedListNode BatchDelete(SinglyLinkedListNode head, int m, int n) {
        ...
        while(tmp != null){
            for(int i = 0; i < (m - 1) && tmp != null; i++) tmp = tmp.next;

            SinglyLinkedListNode aux = (tmp == null) ? null : tmp.next;
            for(int i = 0; i < n && aux != null; i++) aux = aux.next;

            if(tmp != null) tmp.next = aux;
            tmp = aux;
        }
